class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.totalEnemies = this.levelConfig.enemies;
    this.killCount = 0;
  ...
  _spawnEnemy(col, row) {
    ...
    // No per-enemy collider setup needed: group colliders in _setupCollisions() cover all members.
  }
  ...
  _setupCollisions() {
    ...
    // Player vs terrain (single call only — duplicate calls cause double-push lock-up)
    this._setupPlayerCollisions();

    // Player bullets vs terrain
  ...
  _spawnPowerUp() {
    ...
    pu.powerType = type;
    this.tweens.add({
      targets: pu, alpha: 0.3, duration: 400, yoyo: true, repeat: 10,
  ...
  update(time, delta) {
    ...
    if (this.player && this.player.active) {
      this.player.update(this.cursors, this.spaceKey, time, delta, this.bullets, this.mapData); // delta kept for compat
    }
